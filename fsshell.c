...
int displayFiles (fdDir * dirp, int flall, int fllong)
	...
	// printf("Inside displayFiles\n");
	//exit(0); //needs to be removed after readdir works
    printf("\nLS -------------------------------------------\n");
	while (di != NULL) 
		{
			// printf("in while loop display files");
...
int cmd_ls (int argcnt, char *argvec[])
	...
	if (optind < argcnt)
		...
		{
		printf("*ls* no path name specified getcwd.\n");
		char * path = fs_getcwd(cwd, DIRMAX_LEN);	//get current working directory
		printf("*ls* getcwd path returned : %s\n", path);
		fdDir * dirp;
		printf("*ls* fs_opendir\n");
		dirp = fs_opendir (path);
		printf("*ls*  END fs_opendir\n");
...
int cmd_cp (int argcnt, char *argvec[])
	...
	printf("open test src : %d\n", testfs_dest_fd);
	do 
		{
		readcnt = b_read (testfs_src_fd, buf, BUFFERLEN);
		printf("read count : %d\n", readcnt); //read 0 from what file
...
int cmd_mv (int argcnt, char *argvec[])
	...
	if(argcnt != 3){
		printf("Usage: mv file from source to destination\n");
		return -1;
	}
	else{
		...
		if(fs_isFile(argvec[1])){
			if(fs_isDir(argvec[2])){
				...
				PPI * returnOfParsePath1 = parsePath(argvec[2]);
				//printf("Print the name of returnOfParsePath1: %s\n", returnOfParsePath1->name);
				//printf("Print the name of returnOfParsePath1: %s\n", returnOfParsePath1->parentDirPtr[returnOfParsePath1->index].fileName);
				//exit(0);
				//printf("Value is: %s\n", returnOfParsePath1->parentDirPtr[returnOfParsePath1->index]);

				//exit(0);
				DE * returnOfFindEmptyDE;
				if(returnOfParsePath1 != NULL){
					//printf("Are we here\n");
					//exit(0);
					//check if there is an empty directory entry
					char * bufferToOpenDir = malloc(BLOCK_SIZE*NUM_DE_IN_DIR);
					int blocksSpanned = blocksNeeded(BLOCK_SIZE*NUM_DE_IN_DIR);
					LBAread(bufferToOpenDir, blocksSpanned, returnOfParsePath1->parentDirPtr[returnOfParsePath1->index].location);
					printf("Location of directory we want to move: %d\n", returnOfParsePath1->parentDirPtr[returnOfParsePath1->index].location);
					mvDir = malloc(NUM_DE_IN_DIR*sizeof(DE));
					memcpy(mvDir, bufferToOpenDir, NUM_DE_IN_DIR*sizeof(DE));

					returnOfFindEmptyDE = findEmptyDE(mvDir);


					//returnOfFindEmptyDE = findEmptyDE(returnOfParsePath1->parentDirPtr);

					printf("\n\nDirectory name as per retunOfFindEmptyDE: %s\n\n", returnOfFindEmptyDE->fileName);
					//exit(0);
					
					if(returnOfFindEmptyDE==NULL){
				...
				PPI * returnOfParsePath2 = parsePath(argvec[1]);
				//printParsePathReturn(returnOfParsePath2);
				//exit(0);
				//check if our given file exits or not
				if(returnOfParsePath2 != NULL){
					...
					printf("*****************************\n");
					printf("Dir Entry original details:\n"
					"Location: %d\nFileSize: %d\nBlocksSpanned: %d\n"
					"FileName: %s\nFileType: %s\nCreated: %ld\nLastModified: %ld\n",
					returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].location,returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileSize,
					returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].blocksSpanned,returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileName,
					returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileType, returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].created,returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].lastModified);
					printf("*****************************\n");

					returnOfFindEmptyDE->location = returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].location;//-1
					returnOfFindEmptyDE->fileSize = returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileSize;//0
					returnOfFindEmptyDE->blocksSpanned = returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].blocksSpanned;//0
					//returnOfFindEmptyDE->fileName = returnOfParsePath2->parentDirPtr->fileName;
					strcpy(returnOfFindEmptyDE->fileName, returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileName);//strcpy ""
					//returnOfFindEmptyDE->fileType = returnOfParsePath2->parentDirPtr->fileType;
					strcpy(returnOfFindEmptyDE->fileType, returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileType);//""
					returnOfFindEmptyDE->created = returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].created;//0
					returnOfFindEmptyDE->lastModified = returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].lastModified;//0
					printf("*****************************\n");
					printf("Details copied with mv command are:\n"
					"Location: %d\nFileSize: %d\nBlocksSpanned: %d\n"
					"FileName: %s\nFileType: %s\nCreated: %ld\nLastModified: %ld\n",
					returnOfFindEmptyDE->location,returnOfFindEmptyDE->fileSize,
					returnOfFindEmptyDE->blocksSpanned,returnOfFindEmptyDE->fileName,
					returnOfFindEmptyDE->fileType, returnOfFindEmptyDE->created,returnOfFindEmptyDE->lastModified);
					printf("*****************************\n");
					...
					returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].blocksSpanned=0;
					...
					printf("*****************************\n");
					printf("Dir Entry details set to default value:\n"
					"Location: %d\nFileSize: %d\nBlocksSpanned: %d\n"
					"FileName: %s\nFileType: %s\nCreated: %ld\nLastModified: %ld\n",
					returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].location,returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileSize,
					returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].blocksSpanned,returnOfParsePath2->parentDirPtr[returnOfParsePath2->index].fileName,
					...
					printf("Path of file: %s\n",returnOfParsePath2->path);
					printf("Path of directory: %s\n",returnOfParsePath2->path);
					printf("currPath value: %s\n",currPath);
					printf("Arg 1: %s\n", argvec[0]);
					printf("Arg 2: %s\n", argvec[1]);
					printf("Arg 3: %s\n", argvec[2]);
					
					// printf("Are we here in mv\n");
					// exit(0);
					printf("\n\n #\n#\n");

					unsigned char * bufferOfMV = malloc(sizeof(DE)*NUM_DE_IN_DIR);
					
					memcpy(bufferOfMV, returnOfParsePath2->parentDirPtr, returnOfParsePath2->parentDirPtr[0].fileSize);
					int blocksSpannedInMv = blocksNeeded(sizeof(DE)*NUM_DE_IN_DIR);
					printf("blocksSpannedInMv: %d\n", blocksSpannedInMv);
					LBAwrite(bufferOfMV, blocksSpannedInMv, returnOfFindEmptyDE->location);

					
					

					unsigned char * bufferOfMV2 = malloc(sizeof(DE)*NUM_DE_IN_DIR);

					
					blocksSpannedInMv = blocksNeeded(BLOCK_SIZE*NUM_DE_IN_DIR);
					memcpy(bufferOfMV2, mvDir, mvDir[0].fileSize);
					LBAwrite(bufferOfMV2,blocksSpannedInMv, mvDir[0].location);

					// // if (strcmp(returnOfParsePath1->path, currPath) == 0)
					// // {
					// 	//unsigned char * bufferOfMV2 = malloc(BLOCK_SIZE*NUM_DE_IN_DIR);
					// 	DE * mv2 = malloc(NUM_DE_IN_DIR*sizeof(DE));
					// 	DE * tempPointerInMV = mv2;
					// 	//for(int i=0;i<NUM_DE_IN_DIR;i++){
					// 		memcpy(mv2, dir, NUM_DE_IN_DIR*sizeof(DE));
					// 	// 	tempPointerInMV+=sizeof(DE);
					// 	// }
					...
					// if (strcmp(returnOfParsePath2->path, currPath) == 0)
					// {
					// 	printf("\nreloading current directory\n");
					// 	//then we need to reload current directory to reflect changes.
					// 	int retReload = reloadCurrentDir(returnOfParsePath2->parentDirPtr);
					// 	printf("Return of reload: %d\n", retReload);
					// 	printf("\n2)MV: print current directory see if changes took\n");
					// 	//printCurrentDir(dir);
					// }
...
int main (int argc, char * argv[])
	...
	retVal = initFileSystem (volumeSize / blockSize, blockSize); // 10,000,000 volume , 512 block
